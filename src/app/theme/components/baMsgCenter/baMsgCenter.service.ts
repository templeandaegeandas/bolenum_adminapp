...
export class BaMsgCenterService {
  ...
  GetUserNotification(currentPage: number, pageSize: number, sortBy: String, sortOrder: String) {
    ...
    return this
      .http
      .get('/api/v1/user/notification?pageNumber=' + this.pageNumber + '&pageSize=' + pageSize + '&sortBy=' + sortBy + '&sortOrder=' + sortOrder)
      .map(res => res.json());
  }
  ...
  changeReadStatusOfUserNotification(arrayOfNotification: any) {
    return this.http.put('/api/v1/user/notification?arrayOfNotification =' + arrayOfNotification, '')
      .map(res => res.json());
  }
